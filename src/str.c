 ...
 *
 * FIT VUT, IFJ 089
 * Summary: jednoducha knihovna pro praci s nekonecne dlouhymi retezci
 * Notes: Used from official example, added strFirst()
 *
 */
...
char strDelChar(string *s)  // deletes last char in string
{
	if (!s->length)  // string is empty
		return 0;
...
char strFirst(string *s)  // first char in string
{
	return s->str[0];
}

char strLast(string *s)  // last char in string
{
	if (!s->length)
